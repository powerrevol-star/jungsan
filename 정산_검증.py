...
def load_db(file_path: str, partner_name: str) -> pd.DataFrame:
    ...
    exact_mask = df["제휴사명"] == partner_name
    if exact_mask.any():
        df = df[exact_mask].copy()
    else:
        # 정규식 특수문자 이스케이프 후 부분 일치
        escaped = _re.escape(partner_name)
        mask = df["제휴사명"].str.contains(escaped, na=False, regex=True)
        df = df[mask].copy()

    all_partners = pd.read_excel(file_path, sheet_name="대출시트")["MYDA_ORG_NM"].dropna().unique().tolist()
    if df.empty:
        raise ValueError(f"DB에서 '{partner_name}' 데이터를 찾을 수 없습니다.\n"
                         f"존재하는 제휴사: {sorted(all_partners)}")
...
# 상태값 동의어 그룹: (대표값, {동의어 집합}) 튜플 리스트
# 대표값은 첫 번째 원소로 고정. 부분 문자열 매칭도 적용됨.
STATUS_SYNONYM_GROUPS = [
    ("실행", {"실행", "정상", "03-기표(정상)", "기표"}),
    ("철회", {"철회", "취소", "해지", "06-철회"}),
    ("심사중", {"심사중", "처리중"}),
    ("완제", {"완제", "종료", "05-종료(완제)"}),
]
...
def normalize_status(val: str) -> str:
    """
    상태값을 동의어 그룹의 대표값으로 정규화.
    1단계: 정확한 값이 그룹에 있으면 대표값 반환.
    2단계: 그룹 내 키워드가 값에 포함되어 있으면 대표값 반환.
           (예: '03-기표(정상)' → '정상' 포함 → 대표값 '실행' 반환)
    """
    v = str(val).strip()
    # 1단계: 정확한 일치
